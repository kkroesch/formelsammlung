...
\begin{align*}
R_{ser} =& R_{1} + \cdots + R_{n} \qquad \frac{1}{R_{par}} =& \frac{1}{R_{1}} + \cdots + \frac{1}{R_{n}} \\
\frac{1}{C_{ser}} =& \frac{1}{C_{1}} + \cdots + \frac{1}{C_{n}} \qquad C_{par} =& C_{1} + \cdots + C_{n} \\
L_{ser} =& L_{1} + \cdots + L_{n} \qquad \frac{1}{L_{par}} =& \frac{1}{L_{1}} + \cdots + \frac{1}{L_{n}}
\end{align*}
...
\begin{Figure}
 \centering
  \begin{circuitikz}
   ...
   (opamp.-) |- ($(opamp.-)+(0.2,1)$) to[R=$R_2$] ($(opamp.-)+(2.2,1)$) -|
   (opamp.out) to[short,*-] ($(opamp.out)+(.5,0)$) node [right] {$U_{out}$} node [ocirc] {} 
   (opamp.+) to[short]  ($(opamp.-)-(0,0)$) node[left]{$U_{in}$};
  \end{circuitikz}
